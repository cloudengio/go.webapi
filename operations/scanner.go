...
func (sc *Scanner[T]) first(ctx context.Context) (bool, error) {
	var empty T
	nextURL, nextBody, done, err := sc.paginator.Next(empty, nil)
	if err != nil || done {
		return done, err
	}
	sc.ep = NewEndpoint[T](nextURL, sc.opts...)
	go sc.get(ctx, nextURL, nextBody)
	return false, nil
}
...
func (sc *Scanner[T]) Scan(ctx context.Context) bool {
	...
	sc.resp = resp
	go sc.get(ctx, resp.nextURL, resp.nextBody)
	return true
}
...
func (sc *Scanner[T]) get(ctx context.Context, nextURL string, nextBody io.Reader) {
	payload, resp, body, err := sc.ep.getWithResp(ctx, nextURL, nextBody)
	if err != nil {
		sc.ch <- response[T]{response: payload, body: body, last: true, err: err}
		return
	}
	nextURL, nextBody, last, err := sc.paginator.Next(payload, resp)
	if err != nil {
		sc.ch <- response[T]{response: payload, body: body, last: true, err: err}
		return
	}
