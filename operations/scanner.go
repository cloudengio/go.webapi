...
// body) given the response from the previous request.
type Paginator[T any] interface {
	...
	Next(t T, r *http.Response) (url string, header http.Header, body io.Reader, done bool, err error)
...
func (sc *Scanner[T]) first(ctx context.Context) (bool, error) {
	var empty T
	nextURL, nextHeader, nextBody, done, err := sc.paginator.Next(empty, nil)
	if err != nil || done {
		return done, err
	}
	sc.ep = NewEndpoint[T](sc.opts...)
	go sc.get(ctx, nextURL, nextHeader, nextBody)
	return false, nil
}
...
func (sc *Scanner[T]) Scan(ctx context.Context) bool {
	...
	sc.resp = resp
	go sc.get(ctx, resp.nextURL, resp.nextHeader, resp.nextBody)
	return true
}
...
func (sc *Scanner[T]) get(ctx context.Context, nextURL string, nextHeader http.Header, nextBody io.Reader) {
	payload, resp, body, err := sc.ep.getWithResp(ctx, nextURL, nextHeader, nextBody)
	if err != nil {
		sc.ch <- response[T]{response: payload, body: body, last: true, err: err}
		return
	}
	sc.paginator.Save()
	nextURL, nextHeader, nextBody, last, err := sc.paginator.Next(payload, resp)
	if err != nil {
		sc.ch <- response[T]{response: payload, body: body, last: true, err: err}
		return
	}
